...
class Container extends Element_Base {
	...
	public function before_render() {
		...
		if ( ! empty( $settings['link']['url'] ) ) {
			$this->add_link_attributes( '_wrapper', $settings['link'] );
 		}
	...
	protected function register_instance_controls( $settings ) {
		...
		$active_devices = Plugin::$instance->breakpoints::_get_active_devices_list();
		foreach ( $active_devices as $breakpoint_key ) {
			if ( isset( $settings[ 'hide_'.$breakpoint_key ] ) ) {
				$controls[ 'hide_'.$breakpoint_key ] = [
					'label' => 'Hide on...',
					'type' => Controls_Manager::SWITCHER,
					'default' => '',
		...
		if ( isset( $settings['background_background'] ) ) {
			if ( $settings['background_background'] === 'video' ) {
				$controls['background_video_link'] = [
				...
				$controls['background_play_on_mobile'] = [
			...
			if ( $settings['background_background'] === 'slideshow' ) {
				$controls['background_slideshow_gallery'] = [
				...
				$controls['background_slideshow_loop'] = [
				...
				$controls['background_slideshow_slide_duration'] = [
				...
				$controls['background_slideshow_slide_transition'] = [
				...
				$controls['background_slideshow_transition_duration'] = [
				...
				$controls['background_slideshow_lazyload'] = [
				...
				$controls['background_slideshow_ken_burns'] = [
				...
				$controls['background_slideshow_ken_burns_zoom_direction'] = [
		...
		if ( isset( $settings['shape_divider_top'] ) ) {
			...
			$controls['shape_divider_top_negative'] = [
				'label' => esc_html__( 'Invert', 'elementor' ),
				'type' => Controls_Manager::SWITCHER,
				'frontend_available' => true,
				'render_type' => 'none',
				'condition' => [
					'shape_divider_top' => array_keys( Shapes::filter_shapes( 'has_negative' ) ),
				],
			];
		}

		if ( isset( $settings['shape_divider_bottom'] ) ) {
			$controls['shape_divider_bottom'] = [
				'label' => esc_html__( 'Type', 'elementor' ),
				'type' => Controls_Manager::SELECT,
				'render_type' => 'none',
				'frontend_available' => true,
			...
			$controls['shape_divider_bottom_negative'] = [
				'label' => esc_html__( 'Invert', 'elementor' ),
				'type' => Controls_Manager::SWITCHER,
				'frontend_available' => true,
				'render_type' => 'none',
				'condition' => [
					'shape_divider_bottom' => array_keys( Shapes::filter_shapes( 'has_negative' ) ),
				],
			];
		}

		$this->_register_common_instance_controls( $controls, $settings );
