...
abstract class Element_Base extends Controls_Stack {
	...
	protected function register_instance_controls( $settings ) {
		return [];
	}
	...
	protected function add_instance_controls() {
		if ( \vxe_is_runtime() ) {
			$settings = $this->get_settings();
			$instance_controls = $this->register_instance_controls( $settings );

			if ( $this->get_type() === 'widget' ) {
				$instance_controls += Plugin::$instance->widgets_manager->get_widget_types( 'common' )->register_instance_controls( $settings );
			}

			if ( ! empty( $instance_controls ) ) {
				$this->start_controls_section( '_vx_instance_controls' );

				foreach ( $instance_controls as $instance_control_key => $instance_control ) {
					$instance_control['name'] = $instance_control_key;

					if ( ! empty( $instance_control['__responsive'] ) ) {
						$this->add_responsive_control( $instance_control['name'], $instance_control );
					} else {
						$this->add_control( $instance_control['name'], $instance_control );
					}

					$GLOBALS['control_count']++;
					// $GLOBALS['control_list'][] = $instance_control;

					if ( isset( $settings[ $instance_control['name'] ] ) ) {
						$control_obj = Plugin::$instance->controls_manager->get_control( $instance_control['type'] );
						if ( $control_obj instanceof Base_Data_Control ) {
							$settings[ $instance_control['name'] ] = $control_obj->get_value( $instance_control, $settings );
						}
					} else {
						if ( isset( $instance_control['default'] ) ) {
							$settings[ $instance_control['name'] ] = $instance_control['default'];
						} else {
							$control_obj = Plugin::$instance->controls_manager->get_control( $instance_control['type'] );
							if ( $control_obj instanceof Base_Data_Control ) {
								$settings[ $instance_control['name'] ] = $control_obj->get_default_value();
							}
						}
					}
				}
				...
				$this->_instance_controls = $instance_controls;
			}
		}
	}

	protected function remove_instance_controls() {
		if ( ! empty( $this->_instance_controls ) ) {
			$this->remove_control( '_vx_instance_controls' );
			foreach ( $this->_instance_controls as $instance_control_key => $instance_control ) {
				if ( ! empty( $instance_control['__responsive'] ) ) {
					$this->remove_responsive_control( $instance_control_key );
				} else {
					$this->remove_control( $instance_control_key );
				}
			}
		}
	}

	protected function _register_common_instance_controls( &$controls, $settings ) {
		// transforms
		$transform_prefix_class = 'e-';
		$transform_return_value = 'transform';
		foreach ( [ '', '_hover' ] as $tab ) {
			if ( ! empty( $settings['_transform_rotate_popover'.$tab] ) ) {
				$controls['_transform_rotate_popover'.$tab] = [
					'label' => esc_html__( 'Rotate', 'elementor' ),
					'type' => Controls_Manager::POPOVER_TOGGLE,
					'prefix_class' => $transform_prefix_class,
					'return_value' => $transform_return_value,
			...
			if ( ! empty( $settings['_transform_rotateZ_effect'.$tab] ) ) {
				$controls["_transform_rotateZ_effect{$tab}"] = [
					'label' => esc_html__( 'Rotate', 'elementor' ),
					'type' => Controls_Manager::SLIDER,
					'condition' => [
						"_transform_rotate_popover{$tab}!" => '',
			...
			if ( ! empty( $settings['_transform_rotate_3d'.$tab] ) ) {
				$controls["_transform_rotate_3d{$tab}"] = [
					'label' => esc_html__( '3D Rotate', 'elementor' ),
					'type' => Controls_Manager::SWITCHER,
					'condition' => [
						"_transform_rotate_popover{$tab}!" => '',
			...
			if ( ! empty( $settings['_transform_rotateX_effect'.$tab] ) ) {
				$controls["_transform_rotateX_effect{$tab}"] = [
					'label' => esc_html__( 'Rotate X', 'elementor' ),
					'type' => Controls_Manager::SLIDER,
					'condition' => [
						"_transform_rotate_3d{$tab}!" => '',
			...
			if ( ! empty( $settings['_transform_rotateY_effect'.$tab] ) ) {
				$controls["_transform_rotateY_effect{$tab}"] = [
					'label' => esc_html__( 'Rotate Y', 'elementor' ),
					'type' => Controls_Manager::SLIDER,
					'condition' => [
						"_transform_rotate_3d{$tab}!" => '',
			...
			if ( ! empty( $settings['_transform_perspective_effect'.$tab] ) ) {
				$controls["_transform_perspective_effect{$tab}"] = [
					'label' => esc_html__( 'Perspective', 'elementor' ),
					'type' => Controls_Manager::SLIDER,
					'condition' => [
						"_transform_rotate_popover{$tab}!" => '',
			...
			if ( ! empty( $settings['_transform_translate_popover'.$tab] ) ) {
				$controls["_transform_translate_popover{$tab}"] = [
					'label' => esc_html__( 'Offset', 'elementor' ),
					'type' => Controls_Manager::POPOVER_TOGGLE,
					'prefix_class' => $transform_prefix_class,
					'return_value' => $transform_return_value,
			...
			if ( ! empty( $settings['_transform_translateX_effect'.$tab] ) ) {
				$controls["_transform_translateX_effect{$tab}"] = [
					'label' => esc_html__( 'Offset X', 'elementor' ),
					'type' => Controls_Manager::SLIDER,
					'condition' => [
						"_transform_translate_popover{$tab}!" => '',
			...
			if ( ! empty( $settings['_transform_translateY_effect'.$tab] ) ) {
				$controls["_transform_translateY_effect{$tab}"] = [
					'label' => esc_html__( 'Offset Y', 'elementor' ),
					'type' => Controls_Manager::SLIDER,
					'condition' => [
						"_transform_translate_popover{$tab}!" => '',
			...
			if ( ! empty( $settings['_transform_scale_popover'.$tab] ) ) {
				$controls["_transform_scale_popover{$tab}"] = [
					'label' => esc_html__( 'Scale', 'elementor' ),
					'type' => Controls_Manager::POPOVER_TOGGLE,
					'prefix_class' => $transform_prefix_class,
					'return_value' => $transform_return_value,
			...
			if ( ! empty( $settings['_transform_keep_proportions'.$tab] ) ) {
				$controls["_transform_keep_proportions{$tab}"] = [
					'label' => esc_html__( 'Keep Proportions', 'elementor' ),
					'type' => Controls_Manager::SWITCHER,
					'label_on' => esc_html__( 'On', 'elementor' ),
					'label_off' => esc_html__( 'Off', 'elementor' ),
			...
			if ( ! empty( $settings['_transform_scale_effect'.$tab] ) ) {
				$controls["_transform_scale_effect{$tab}"] = [
					'label' => esc_html__( 'Scale', 'elementor' ),
					'type' => Controls_Manager::SLIDER,
					'condition' => [
						"_transform_scale_popover{$tab}!" => '',
			...
			if ( ! empty( $settings['_transform_scaleX_effect'.$tab] ) ) {
				$controls["_transform_scaleX_effect{$tab}"] = [
					'label' => esc_html__( 'Scale X', 'elementor' ),
					'type' => Controls_Manager::SLIDER,
					'condition' => [
						"_transform_scale_popover{$tab}!" => '',
			...
			if ( ! empty( $settings['_transform_scaleY_effect'.$tab] ) ) {
				$controls["_transform_scaleY_effect{$tab}"] = [
					'label' => esc_html__( 'Scale Y', 'elementor' ),
					'type' => Controls_Manager::SLIDER,
					'condition' => [
						"_transform_scale_popover{$tab}!" => '',
			...
			if ( ! empty( $settings['_transform_skew_popover'.$tab] ) ) {
				$controls["_transform_skew_popover{$tab}"] = [
					'label' => esc_html__( 'Skew', 'elementor' ),
					'type' => Controls_Manager::POPOVER_TOGGLE,
					'prefix_class' => $transform_prefix_class,
					'return_value' => $transform_return_value,
			...
			if ( ! empty( $settings['_transform_skewX_effect'.$tab] ) ) {
				$controls["_transform_skewX_effect{$tab}"] = [
					'label' => esc_html__( 'Skew X', 'elementor' ),
					'type' => Controls_Manager::SLIDER,
					'condition' => [
						"_transform_skew_popover{$tab}!" => '',
			...
			if ( ! empty( $settings['_transform_skewY_effect'.$tab] ) ) {
				$controls["_transform_skewY_effect{$tab}"] = [
					'label' => esc_html__( 'Skew Y', 'elementor' ),
					'type' => Controls_Manager::SLIDER,
					'condition' => [
						"_transform_skew_popover{$tab}!" => '',
			...
			if ( ! empty( $settings['_transform_flipX_effect'.$tab] ) ) {
				$controls["_transform_flipX_effect{$tab}"] = [
					'label' => esc_html__( 'Flip Horizontal', 'elementor' ),
					'type' => Controls_Manager::CHOOSE,
					'prefix_class' => $transform_prefix_class,
					'frontend_available' => true,
			...
			if ( ! empty( $settings['_transform_flipY_effect'.$tab] ) ) {
				$controls["_transform_flipY_effect{$tab}"] = [
					'label' => esc_html__( 'Flip Vertical', 'elementor' ),
					'type' => Controls_Manager::CHOOSE,
					'prefix_class' => $transform_prefix_class,
					'frontend_available' => true,
