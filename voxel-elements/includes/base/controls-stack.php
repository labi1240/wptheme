...
abstract class Controls_Stack extends Base_Object {
	...
	public function get_active_controls( array $controls = null, array $settings = null ) {
		\Elementor\Plugin::$instance->modules_manager->get_modules( 'dev-tools' )->deprecation->deprecated_function( __METHOD__, '3.0.0' );
	...
	public function add_control( $id, array $args, $options = [] ) {
		...
		if ( isset( $args['scheme'] ) ) {
			$args['global'] = [
				'default' => \Elementor\Plugin::$instance->kits_manager->convert_scheme_to_global( $args['scheme'] ),
			];
		...
		return \Elementor\Plugin::$instance->controls_manager->add_control_to_stack( $this, $id, $args, $options );
	}
	...
	public function remove_control( $control_id ) {
		return \Elementor\Plugin::$instance->controls_manager->remove_control_from_stack( $this->get_unique_name(), $control_id );
	}
	...
	public function update_control( $control_id, array $args, array $options = [] ) {
		$is_updated = \Elementor\Plugin::$instance->controls_manager->update_control_in_stack( $this, $control_id, $args, $options );
	...
	public function get_stack() {
		$stack = \Elementor\Plugin::$instance->controls_manager->get_element_stack( $this );

		if ( null === $stack ) {
			$this->init_controls();

			return \Elementor\Plugin::$instance->controls_manager->get_element_stack( $this );
		}
	...
	final public function add_group_control( $group_name, array $args = [], array $options = [] ) {
		$group = \Elementor\Plugin::$instance->controls_manager->get_control_groups( $group_name );
	...
	final public function add_responsive_control( $id, array $args, $options = [] ) {
		...
		if ( ! empty( $options['overwrite'] ) ) {
			$existing_control = \Elementor\Plugin::$instance->controls_manager->get_control_from_stack( $this->get_unique_name(), $id );
	...
	final public function get_config() {
		if ( null === $this->config ) {
			...
			if ( $this->has_own_method( '_get_initial_config', self::class ) ) {
				\Elementor\Plugin::$instance->modules_manager->get_modules( 'dev-tools' )->deprecation->deprecated_function( '_get_initial_config', '2.9.0', __CLASS__ . '::get_initial_config()' );
	...
	public function get_active_settings( $settings = null, $controls = null ) {
		...
		$controls_objs = \Elementor\Plugin::$instance->controls_manager->get_controls();
	...
	public function get_settings_for_display( $setting_key = null ) {
		if ( ! $this->parsed_active_settings ) {
			if ( _ELEMENTOR_PRO_IS_ACTIVE ) {
				$this->parsed_active_settings = $this->get_active_settings( $this->get_parsed_dynamic_settings(), $this->get_controls() );
			} else {
				$this->parsed_active_settings = $this->get_active_settings( $this->get_settings(), $this->get_controls() );
			}
		}
	...
	protected function get_responsive_control_device_suffix( $control ) {
		\Elementor\Plugin::$instance->modules_manager->get_modules( 'dev-tools' )->deprecation->deprecated_function( __METHOD__, '3.7.6', 'Elementor\Controls_Manager::get_responsive_control_device_suffix()' );
	...
	public function is_control_visible_deep( $control, $values = null, $controls = null ) {
		if ( null === $values ) {
			$values = $this->get_settings();
		}
		...
		foreach ( $control['condition'] as $condition_key => $condition_value ) {
			...
			if ( ! $instance_value ) {
				...
				while ( $parent ) {
					$instance_value = $values[ $parent ] ?? null;
	...
	public function print_template() {
		...
		if ( $this->has_own_method( '_content_template', self::class ) ) {
			\Elementor\Plugin::$instance->modules_manager->get_modules( 'dev-tools' )->deprecation->deprecated_function( '_content_template', '2.9.0', __CLASS__ . '::content_template()' );
	...
	protected function _register_controls() {
		\Elementor\Plugin::$instance->modules_manager->get_modules( 'dev-tools' )->deprecation->deprecated_function( __METHOD__, '3.1.0', 'register_controls()' );
	...
	protected function get_init_settings() {
		if ( ! isset( $GLOBALS['control_count'] ) ) {
			$GLOBALS['control_count'] = 0;
		}
		...
		foreach ( $this->get_controls() as $control ) {
			...
			if ( isset( $settings[ $control['name'] ] ) ) {
				...
				if ( isset( $control['default'] ) ) {
					...
					$control_obj = $controls_objs[ $control['type'] ] ?? null;
					if ( $control_obj instanceof \Elementor\Base_Data_Control ) {
						$settings[ $control['name'] ] = $control_obj->get_default_value();
					}
				}
			}
		}
	...
	protected function _get_initial_config() {
		\Elementor\Plugin::$instance->modules_manager->get_modules( 'dev-tools' )->deprecation->deprecated_function( __METHOD__, '2.9.0', 'get_initial_config()' );
	...
	protected function render_by_mode() {
		if ( \Elementor\Plugin::$instance->frontend->is_static_render_mode() ) {
			$this->render_static();
	...
	protected function _content_template() {
		\Elementor\Plugin::$instance->modules_manager->get_modules( 'dev-tools' )->deprecation->deprecated_function( __METHOD__, '2.9.0', 'content_template()' );
	...
	protected function init_controls() {
		\Elementor\Plugin::$instance->controls_manager->open_stack( $this );

		// TODO: This is for backwards compatibility starting from 2.9.0
		// This `if` statement should be removed when the method is removed
		if ( $this->has_own_method( '_register_controls', self::class ) ) {
			\Elementor\Plugin::$instance->modules_manager->get_modules( 'dev-tools' )->deprecation->deprecated_function( '_register_controls', '3.1.0', __CLASS__ . '::register_controls()' );

			$this->_register_controls();
		} else {
			if ( method_exists( $this, 'register_runtime_controls' ) && \vxe_is_runtime() ) {
				$this->start_controls_section( '_vx_runtime_controls' );
				$this->register_runtime_controls();
	...
	protected function handle_control_position( array $args, $control_id, $overwrite ) {
		...
		} elseif ( empty( $args['section'] ) && ( ! $overwrite || is_wp_error( \Elementor\Plugin::$instance->controls_manager->get_control_from_stack( $this->get_unique_name(), $control_id ) ) ) ) {
	...
	protected function _init( $data ) {
		\Elementor\Plugin::$instance->modules_manager->get_modules( 'dev-tools' )->deprecation->deprecated_function( __METHOD__, '2.9.0', 'init()' );
	...
	private function sanitize_settings( array $settings, array $controls = [] ) {
		...
		foreach ( $controls as $control ) {
			...
			$tag_text_data = \Elementor\Plugin::$instance->dynamic_tags->tag_text_to_tag_data( $value_to_check );

			if ( ! \Elementor\Plugin::$instance->dynamic_tags->get_tag_info( $tag_text_data['name'] ) ) {
	...
	public function __construct( array $data = [] ) {
		if ( $data ) {
			...
			if ( $this->has_own_method( '_init', self::class ) ) {
				\Elementor\Plugin::$instance->modules_manager->get_modules( 'dev-tools' )->deprecation->deprecated_function( '_init', '2.9.0', __CLASS__ . '::init()' );
