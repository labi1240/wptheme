...
class Widget_Divider extends Widget_Base {
	...
	private function build_svg() {
		...
		if ( 'line' !== $selected_pattern['group'] ) {
			...
			$attr['fill'] = 'none';
			$attr['stroke'] = 'black';
			$attr['stroke-width'] = $settings['weight']['size'] ?? 1;
	...
	protected function register_instance_controls( $settings ) {
		...
		if ( isset( $settings['style'] ) && isset( $pattern_spacing_flag[ $settings['style'] ] ) ) {
			$controls['pattern_spacing_flag'] = [
				'type' => Controls_Manager::HIDDEN,
				'default' => 'no-spacing',
				'prefix_class' => 'elementor-widget-divider--',
				'render_type' => 'template',
		...
		if ( isset( $settings['style'] ) && isset( $pattern_round_flag[ $settings['style'] ] ) ) {
			$controls['pattern_round_flag'] = [
				'type' => Controls_Manager::HIDDEN,
				'default' => 'bg-round',
				'prefix_class' => 'elementor-widget-divider--',
			];
		...
		if ( isset( $settings['look'] ) && $settings['look'] === 'line_icon' ) {
			$controls['icon'] = [
				'label' => esc_html__( 'Icon', 'elementor' ),
				'type' => Controls_Manager::ICONS,
				'default' => [
		...
		if ( isset( $settings['look'] ) && $settings['look'] === 'line_text' ) {
			$controls['text'] = [
				'label' => esc_html__( 'Text', 'elementor' ),
				'type' => Controls_Manager::TEXT,
				'condition' => [
