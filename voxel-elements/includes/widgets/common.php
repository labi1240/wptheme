...
class Widget_Common extends Widget_Base {
	...
	public function register_instance_controls( $settings ) {
		...
		foreach ( $active_devices as $breakpoint_key ) {
			...
			if ( isset( $settings['_element_width_'.$breakpoint_key] ) ) {
				$add_element_width = true;
			}
		}

		if ( $add_element_width || isset( $settings['_element_width'] ) ) {
			$controls['_element_width'] = [
				'label' => esc_html__( 'Width', 'elementor' ),
				'type' => Controls_Manager::SELECT,
				'default' => '',
				'options' => [
		...
		if ( isset( $settings['_background_background'] ) && $settings['_background_background'] === 'video' ) {
			$controls['_background_video_link'] = [
			...
			$controls['_background_play_on_mobile'] = [
			...
			$controls['_background_play_once'] = [
